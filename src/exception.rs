...
fn data_abort_handler(registers: &mut Registers, esr_el2: u64) {
    // ISV は1つのレジスタに対するメモリの読み書きの場合に1, それ以外の場合は0になる。
    // 今回は1つのレジスタに対するメモリの読み書きのみサポートする。
    ...
    // HPFAR_EL2_FIPA はData Abortの原因となった中間物理アドレスのページ部が格納されている。
    // FAR_EL2 はData Abortの原因となった仮想アドレスが格納されているため、そのオフセット部を取る。
